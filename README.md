...
```bash
npm install @xdanangelxoqenpm/molestiae-qui-repellat
```
...
```javascript
require ('@xdanangelxoqenpm/molestiae-qui-repellat').nice // .nice for unsafe String extensions
...
```javascript
import { green, inverse, bgLightCyan, underline, dim } from '@xdanangelxoqenpm/molestiae-qui-repellat'
```
```javascript
const { green, inverse, bgLightCyan, underline, dim } = require ('@xdanangelxoqenpm/molestiae-qui-repellat')
```
...
```javascript
import { @xdanangelxoqenpm/molestiae-qui-repellat, ParsedSpan } from '@xdanangelxoqenpm/molestiae-qui-repellat' // along with type definitions
```
```javascript
import @xdanangelxoqenpm/molestiae-qui-repellat from '@xdanangelxoqenpm/molestiae-qui-repellat'
```

### Nice Mode (not recommended)

```javascript
const ansi = require ('@xdanangelxoqenpm/molestiae-qui-repellat').nice
```

The `('@xdanangelxoqenpm/molestiae-qui-repellat').nice` export defines styling APIs on the `String` prototype directly. It uses an ad-hoc DSL (sort of) for infix-style string coloring. The `nice` is convenient, but not safe, avoid using it in public modules, as it alters global objects, and that might cause potential hard-to-debug compatibility issues.
...
```javascript
const { names } = require ('@xdanangelxoqenpm/molestiae-qui-repellat')
...
```javascript
const { strip } = require ('@xdanangelxoqenpm/molestiae-qui-repellat')
...
```javascript
const { isEscaped, green } = require ('@xdanangelxoqenpm/molestiae-qui-repellat')
...
```javascript
const { parse } = require ('@xdanangelxoqenpm/molestiae-qui-repellat')
...
```javascript
const ansi = require ('@xdanangelxoqenpm/molestiae-qui-repellat')
...
```javascript
const { bgGreen, red, parse } = require ('@xdanangelxoqenpm/molestiae-qui-repellat')
...
## Projects That Use `@xdanangelxoqenpm/molestiae-qui-repellat`
